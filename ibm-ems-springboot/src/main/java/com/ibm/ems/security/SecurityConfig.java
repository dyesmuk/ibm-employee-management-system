...
public class SecurityConfig {
	...
	private static final String[] PUBLIC_PATHS = { "/api/v1/auth/login", "/swagger-ui/**", "/swagger-ui.html",
			"/v3/api-docs/**", "/actuator/health", "/actuator/info" };
	...
	public SecurityFilterChain filterChain(HttpSecurity http, JwtFilter jwtFilter) throws Exception {
		...
		http
				...
				.headers(headers -> headers.frameOptions(frame -> frame.deny()) // A01: Prevent clickjacking
						...
						.contentSecurityPolicy(csp -> csp
								.policyDirectives("default-src 'self'; " + "script-src 'self' 'unsafe-inline'; " + // needed
																													// for
																													// Swagger
																													// UI
	...
	/**
	 * CORS configuration. DevSecOps: Explicitly whitelist allowed origins. Never
	 * use allowedOrigins("*") — it allows any domain to make credentialed
	 * cross-origin requests to the banking API.
	 */
	@Bean
	public CorsConfigurationSource corsConfigurationSource() {
		CorsConfiguration config = new CorsConfiguration();
		// Training: localhost only. Production: replace with actual front-end domain.
		config.setAllowedOrigins(List.of("http://localhost:5172", "http://localhost:5173", "http://localhost:3000",
				"http://localhost:8080"));
	...
	/**
	 * BCrypt password encoder — cost factor 12. DevSecOps (A02): Cost 12 means
	 * ~250ms per hash — expensive for attackers, acceptable for login. NEVER use
	 * MD5, SHA-1, or plain text for passwords.
	 */
	@Bean
